...
def main() -> None:
    ...
    group_name = env("TESTFLIGHT_GROUP_NAME", "Evo Voice LAN Internal")
    timeout_seconds = int(env("BUILD_WAIT_SECONDS", "1800"))
    whats_new = env(
        "WHAT_TO_TEST",
        "Evo Voice LAN：全新 iOS App，用于测试 iPhone -> 192.168.0.11 本机后端 -> 192.168.0.13 模型/语音服务的局域网链路速度。默认关闭联网搜索，便于观察 LLM 首 token、SSE 流式和 TTS 延迟。",
    )
