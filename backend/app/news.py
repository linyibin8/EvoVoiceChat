...
async def search_latest_news(
    ...
    for provider in enabled_search_providers():
        ...
        try:
            ...
            if len(provider_results) < min(3, limit) and domains:
                unrestricted = await _search_provider(provider, base_query, [], limit - len(results))
                provider_results = _merge_results(provider_results, unrestricted, limit - len(results))
